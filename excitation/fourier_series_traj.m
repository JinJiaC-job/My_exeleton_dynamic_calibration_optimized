%% fourier_series_traj
function [q, qd, qdd] = fourier_series_traj(traj_param, dof, time, traj_wf, traj_order)
% @brief: 基于傅立叶级数生成轨迹，对于每个关节：q0 (only one), alpha (#order), beta (#order)
%          #(1+2*traj_order) parameters in total
...
for ii = 1:dof
   ...
   q(ii) = traj_param(m + order_prod_2 + 1);%q0
   for jj = 1:traj_order
