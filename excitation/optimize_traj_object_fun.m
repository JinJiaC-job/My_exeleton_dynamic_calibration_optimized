...
% 	[q, qd, qdd] = mixed_traj(x, dof, (k-1)*traj_Ts, traj_wf, traj_order);
    [q, qd, qdd] = fourier_series_traj(x, dof, (k-1)*traj_Ts, traj_wf, traj_order);
...
[opt_q0, opt_qd0, opt_qdd0] = fourier_series_traj(x, dof, 0, traj_wf, traj_order);
[opt_qn, opt_qdn, opt_qddn] = fourier_series_traj(x, dof, traj_n*traj_Ts, traj_wf, traj_order);
q_init = [95.35; 0; 0; -pi/2; pi/2; 0];    % initial configuration
disp(['initial error, q: ', num2str(sum(opt_q0 - q_init)), ', qd: ', num2str(sum(opt_qd0)), ', qdd: ', num2str(sum(opt_qdd0))]);
disp(['terminal error, q: ', num2str(sum(opt_qn - q_init)), ', qd: ', num2str(sum(opt_qdn)), ', qdd: ', num2str(sum(opt_qddn))]);
